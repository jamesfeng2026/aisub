...
讯飞「录音文件转写大模型」作为云 ASR 服务商接入：异步订单制（上传建单 → 轮询取结果）封装在 service 层，对云引擎适配器保持与同步服务商一致的 `AsrTranscriber` 合同；提供语种档位（autodialect/autominor）选择、上传前语言守卫、词级时间戳解析、跨会话订单续查与零成本连通性自测。
