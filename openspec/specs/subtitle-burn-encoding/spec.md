# Spec: subtitle-burn-encoding

## Purpose

硬字幕烧录的视频编码策略：按编码方式显式选定编码器（CPU libx264 / 硬件 VideoToolbox·NVENC·QSV），画质档位到编码参数的映射（含 4K 自适应偏移与 Intel VT 码率分支），MP4 faststart，硬件路径 8-bit 像素格式兜底与失败自动回退。
