...
/** 进度 key：firered:<modelId>，与 funasr:<id> / qwen:<id> 同构，渲染层按前缀路由。 */
...
/**
 * FireRedASR 模型下载器：整包（tar.bz2）下载 + 解包，或 ModelScope 逐文件直下，
 * 落 userData/models/firered/<id>/。复用 downloadFileParallel（断点续传 + 多连接 + 取消），
 * 解包用 extractArchive（独立进程 system tar）。与 QwenModelDownloader 同构。
 */
